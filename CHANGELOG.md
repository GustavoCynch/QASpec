...
- **Planning home** — QASpec resolves only `qaspec/` in consumer projects. Legacy `openspec/` layouts are no longer recognized.
- **Per-change metadata** — Change metadata files are `.qaspec.yaml` (was `.openspec.yaml`). No migration shim.
- **Environment variables** — `QASPEC_CONCURRENCY` and `QASPEC_NO_COMPLETIONS` replace `OPENSPEC_CONCURRENCY` and `OPENSPEC_NO_COMPLETIONS`. `OPENSPEC_TELEMETRY` is removed with the telemetry module.
- **Global config path** — User config lives under `~/.config/qaspec/` (XDG-resolved), not `~/.config/openspec/`.
- **Telemetry removed** — QASpec sends no usage analytics and shows no first-run telemetry notice.
- **Fork-compat removed** — Upstream OpenSpec coexistence, legacy artifact cleanup, and one-time profile migration are deleted from `qaspec init` / `qaspec update`.
- **Slim publish flow** — `/qsx:publish` no longer writes `publish-plan.md` or `execution-context.md`. The pre-upload review is an in-chat summary derived from `testcases.md`; no publish log is written after upload — published cases are marked `- [x]` in `testcases.md`, which is the only local publish tracking. The TCMS target (provider, project code, base URL) lives in a `tcms` block in `qaspec/config.yaml`, discovered and persisted on first publish when absent. Legacy `execution-context.md` in a change is read once and offered for migration to config; legacy `publish-plan.md` files are ignored. If you edited `publish-plan.md` before confirm, edit `testcases.md` or state exclusions in chat instead.
- **Publish log removed** — `/qsx:publish` no longer writes `publish-log.md`. Published cases are tracked only by `- [x]` checkboxes in `testcases.md`. On re-run, the agent reconciles unchecked cases against existing Qase cases by title before creating and never blind-creates, so an interrupted publish resumes without duplicating cases. Any legacy `publish-log.md` in a change directory is ignored.
