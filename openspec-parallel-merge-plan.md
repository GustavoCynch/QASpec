# QASpec Parallel Delta Remediation Plan
...
4. **Document interim manual mitigation.**
   - Update in-repo spec guidance (`openspec/AGENTS.md` in this repository’s spec tree) and product docs so contributors know to rerun `qaspec change sync` (see Phase 1) whenever another change lands.
...
1. **Introduce `qaspec change sync <id>` (or `rebase`).**
...
2. **Enrich validator messages.**
   - `qaspec validate` should flag unresolved conflict markers or fingerprint mismatches so errors appear early in the workflow.
...
3. **Integrate with Git directly.**
   - Offer optional `qaspec branch` scaffolding that aligns spec changes with Git branches, letting teams leverage Git’s conflict editor for the markdown IR.

_Outcome:_ QASpec graduates from replace-based updates to a resilient, intent-preserving spec management platform.
...
1. Prototype fingerprint capture during `qaspec change validate` and block archive on mismatches.
2. Ship `qaspec change sync` with line-based diff3 merging and conflict markers.
