...
- New `qaspec tcms set/show --change <name>` commands persist and resolve the target (provider, project, baseUrl) in the change's `.openspec.yaml`; the `tcms` block in `qaspec/config.yaml` is now optional user-managed defaults that no workflow or command writes.
