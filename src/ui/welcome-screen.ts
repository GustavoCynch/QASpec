...
function getWelcomeText(): string[] {
  return [
    chalk.white.bold('Welcome to QASpec'),
    chalk.dim('A lightweight spec-driven framework'),
    '',
    chalk.white('This setup will configure:'),
    chalk.dim('  • Agent Skills for AI tools'),
    chalk.dim('  • /qas:* slash commands (QA workflow)'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('/qas:explore')}   ${chalk.dim('Think before the formal cycle')}`,
    `  ${chalk.yellow('/qas:analyze')}   ${chalk.dim('Analysis artifact')}`,
    `  ${chalk.yellow('/qas:matrix')}    ${chalk.dim('Test matrix')}`,
    `  ${chalk.yellow('/qas:publish')}   ${chalk.dim('Publish to Qase')}`,
    '',
