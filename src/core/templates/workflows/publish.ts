...
10. After user confirms (with gate token cited): re-read \`testcases.md\`; read **Preconditions** and **Steps** under each unchecked case; **omit-on-unmapped** — send only Qase fields in the mapping table, never infer severity/priority/type; per case MCP create → mark that case \`- [x]\` in \`testcases.md\` immediately after the successful create call (no local Qase IDs recorded); on re-run reconcile unchecked cases against Qase by title before creating — never blind-create, and cases already present in Qase are marked \`- [x]\` without a duplicate create call.
...
**Guardrails:** target resolution (steps 6–7), publish gate (step 8), and confirmation halt (step 9) are separate from MCP upload; v1 TCMS is Qase only. Do not write \`execution-context.md\`, \`publish-plan.md\`, \`publish-log.md\`, or the \`tcms\` block in \`qaspec/config.yaml\`.`;
