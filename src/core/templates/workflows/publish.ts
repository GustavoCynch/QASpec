...
7. Write or update \`execution-context.md\` and \`publish-plan.md\` (unchecked rows from \`testmatrix.md\`, suites, target project). Use schema templates when creating new files.
...
9. After user confirms: read Qase MCP tool schemas (\`create_suite\`, \`create_case\`, \`bulk_create_cases\` if present); validate matrix against rules; MCP upload; write \`publish-log.md\`; mark each published row \`- [x]\` in \`testmatrix.md\`.
