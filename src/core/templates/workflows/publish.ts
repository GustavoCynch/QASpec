...
Run QASpec **publish** (Phase 3). Prepare Qase publish artifacts, get user confirmation, then upload approved \`testcases.md\` via MCP.
...
7. Write or update \`execution-context.md\` and \`publish-plan.md\` from unchecked cases in \`testcases.md\`, including each case's **Preconditions** and **Steps** blocks when present (not title-only summaries). Use schema templates when creating new files.
