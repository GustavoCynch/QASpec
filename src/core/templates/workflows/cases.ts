...
2. Read \`analysis.md\` in full (mandatory first read). If material clarifications exist only in chat (defect vs expected, scope cuts) and are missing from \`analysis.md\`, update \`analysis.md\` first (especially **Validated clarifications**) — do not draft cases/specs until analysis reflects them.
3. Read \`qaspec/references/qase_test_case_rules.md\`.
4. For each capability in \`analysis.md\`, read \`qaspec/specs/<capability>/spec.md\` when present (baseline for MODIFIED deltas).
...
6. **If cases flag is true:** run **two parallel blind Task** subagents; paste the **full** \`analysis.md\` into each prompt under **Validated analysis (binding)**; then fetch PR/diff per analyst brief. Merge into one case list and aligned delta specs per \`rules.test-cases\` and \`rules.specs\`.
7. **Merge rules (dual analysts only):** union by intent; dedupe only when behavior and boundaries match — not when titles merely look similar. Drop any analyst draft that contradicts \`analysis.md\`.
8. **Self-audit before output:** every case and requirement traceable to \`analysis.md\`; defects test corrected behavior; explicit BVA; no comma/group explosion; readable narrative; API blocking when applicable; every step traceable to a read source unless marked as a documented gap.
...
User-requested edits after halt: if the change affects agreed behavior or defect vs expected, update \`analysis.md\` first, then \`testcases.md\` and affected \`specs/**/*.md\` in the same conversation.`;
