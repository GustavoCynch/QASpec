...
/**
 * Generates Fish completion scripts for the QASpec CLI.
...
export class FishGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Fish completion script for QASpec CLI
  ...
  private generateFlagCompletion(flag: FlagDefinition, condition: string): string[] {
    ...
    if (flag.takesValue && flag.values) {
      ...
      for (const value of flag.values) {
        if (shortFlag) {
          lines.push(
            `complete -c qaspec -n '${condition}' -s ${flag.short} -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        } else {
          lines.push(
            `complete -c qaspec -n '${condition}' -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        }
      }
      ...
      if (shortFlag) {
        lines.push(
          `complete -c qaspec -n '${condition}' -s ${flag.short} -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c qaspec -n '${condition}' -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      }
    } else {
      // Boolean flag
      if (shortFlag) {
        lines.push(
          `complete -c qaspec -n '${condition}' -s ${flag.short} -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c qaspec -n '${condition}' -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      }
    }
