...
PRs are test targets, not ground truth.`,
    ...
    rules: {
      analyze: [
        ...
        'When review flag true: synthesize Agreed / Single-analyst / Contradiction in analysis.md Synthesis notes',
        ...
        'List Affected capabilities in kebab-case; read existing qaspec/specs/<capability>/spec.md when present (previously agreed behavior; baseline for MODIFIED deltas)',
        ...
        'End with exactly one halt question in project language covering both analysis.md and the delta specs; do not continue to cases in the same message',
        'After halt response: persist clarifications in Validated clarifications and update affected specs/**/*.md; never leave approvals only in chat',
      ...
      'test-cases': [
        ...
        'Default: orchestrator-only (workflow.multipleSubagents.cases: false) — draft cases without Task subagents',
        ...
        'Self-audit before halt: every requirement scenario in the delta specs covered by at least one case; every step traceable to a source unless marked as documented gap',
        'End with exactly one approval question covering the case list',
      ],
      specs: [
      ...
      apply: [
        ...
        'Block publish when testcases exists but change specs/ is empty — direct user to /qsx:analyze first (delta specs are co-produced there)',
        'Resolve TCMS target (provider, project code, base URL) from config tcms block; when absent, discover Qase projects via MCP or ask for project code in one halt, offer create-new when MCP supports it, and persist chosen target to qaspec/config.yaml tcms block (announce the edit)',
        'When config lacks tcms and change has legacy execution-context.md, read project code and base URL from it and offer persisting to config; ignore legacy publish-plan.md',
        'Present in-chat publish summary (target, suites with unchecked-case counts, warnings) derived from testcases.md; do not write prepare files',
