...
PRs are test targets, not ground truth.`,
    ...
    rules: {
      ...
      'test-cases': [
        ...
        'Re-read qaspec/references/tcms_case_rules.md before drafting cases',
      ...
      apply: [
        'Re-read qaspec/references/tcms_case_rules.md before the first TCMS MCP call',
        ...
        'Read Preconditions and Steps from each case block in testcases.md for TCMS payloads — do not derive steps from title alone',
        'Omit-on-unmapped: send only TCMS fields in the mapping table; never infer severity/priority/type',
        'Present in-chat publish summary with one representative full case payload; do not write prepare files',
        'End with exactly one confirmation halt after the summary; invoke the TCMS MCP only after user confirms publish — never upload in the same message as target selection or tcms persistence',
        'Do not echo secrets or PII in chat or TCMS fields; stop upload path if detected',
        'Per case: MCP create → mark each published case - [x] in testcases.md immediately after the successful create call; no local TCMS IDs are recorded',
        'On re-run: reconcile unchecked cases against the TCMS by title before creating — never blind-create; cases already present in the TCMS are marked - [x] without a duplicate create call',
      ],
    },
  };
}
