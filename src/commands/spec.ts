...
import { formatPlanningRelativePath, joinPlanningPath } from '../core/planning-dir.js';
...
function filterSpec(spec: Spec, options: ShowOptions): Spec {
  ...
  const metadata = spec.metadata ?? { version: '1.0.0', format: 'qaspec' as const };
...
export class SpecCommand {
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    const specPath = joinPlanningPath(projectRoot, 'specs', specId, 'spec.md');
    if (!existsSync(specPath)) {
      throw new Error(`Spec '${specId}' not found at ${formatPlanningRelativePath(projectRoot, 'specs', specId, 'spec.md')}`);
    }
    ...
    if (options.json) {
      ...
      const output = {
        ...
        metadata: parsed.metadata ?? { version: '1.0.0', format: 'qaspec' as const },
      };
...
export function registerSpecCommand(rootProgram: typeof program) {
  const specCommand = rootProgram
    .command('spec')
    .description('Manage and view QASpec specifications');

  // Deprecation notice for noun-based commands
  specCommand.hook('preAction', () => {
    console.error('Warning: The "qaspec spec ..." commands are deprecated. Prefer verb-first commands (e.g., "qaspec show", "qaspec validate --specs").');
  });
  ...
  specCommand
    ...
    .action((options: { json?: boolean; long?: boolean }) => {
      try {
        const specsDir = joinPlanningPath(process.cwd(), 'specs');
