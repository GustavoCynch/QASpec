...
export async function selectWorkspaceRootForCommand(
  ...
  if (!currentWorkspaceRoot) {
    throw new WorkspaceCliError(
      `No QASpec workspace found at '${workspaceRoot}'.`,
      'workspace_not_found',
      {
        target: 'workspace.root',
        fix: 'Pass a path inside an QASpec workspace.',
      }
    );
  }
...
export async function selectWorkspaceForCommand(
  ...
  if (options.workspace) {
    ...
    if (!registryRoot) {
      throw new WorkspaceCliError(
        `Unknown QASpec workspace '${workspaceName}'.`,
        'workspace_not_found',
        {
          target: 'workspace.name',
          fix: 'Run qaspec workspace list to see known workspaces.',
        }
      );
    }
  ...
  if (entries.length === 0) {
    throw new WorkspaceCliError(
      "No known QASpec workspaces. Run 'qaspec workspace setup' first.\nAfter at least one workspace is known locally, you can also pass --workspace <name>.",
      'no_known_workspaces',
      {
        target: 'workspace.name',
        fix: 'qaspec workspace setup',
      }
    );
  }
  ...
  if (options.json || resolveNoInteractive(options) || !isInteractive(options)) {
    ...
    const fix = usesPositionalName
      ? `qaspec workspace ${commandName} <name>`
      : `qaspec workspace ${commandName} --workspace <name>`;

    throw new WorkspaceCliError(
      usesPositionalName
        ? `Multiple QASpec workspaces are known. Known workspaces: ${knownNames}. Pass a workspace name.`
        : `Multiple QASpec workspaces are known. Known workspaces: ${knownNames}. Pass --workspace <name>.`,
