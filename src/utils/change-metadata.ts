...
export const METADATA_FILENAME = '.qaspec.yaml';
...
/**
 * Writes change metadata to .qaspec.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .qaspec.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .qaspec.yaml metadata (if exists)
 * 3. Schema from qaspec/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/qaspec/changes/change-name)
