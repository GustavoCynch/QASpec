...
describe('global-config', () => {
  ...
  describe('constants', () => {
    it('should export correct directory name', () => {
      expect(GLOBAL_CONFIG_DIR_NAME).toBe('qaspec');
    });
  ...
  describe('getGlobalConfigDir', () => {
    it('should use XDG_CONFIG_HOME when set', () => {
      ...
      expect(result).toBe(path.join(tempDir, 'qaspec'));
    });
    ...
    it('should fall back to ~/.config on Unix/macOS without XDG_CONFIG_HOME', () => {
      ...
      // On non-Windows, should use ~/.config/qaspec
      if (os.platform() !== 'win32') {
        expect(result).toBe(path.join(os.homedir(), '.config', 'qaspec'));
      }
    });
    ...
    it('should use APPDATA on Windows when XDG_CONFIG_HOME is not set', () => {
      ...
      if (os.platform() === 'win32') {
        ...
        if (appData) {
          const result = getGlobalConfigDir();
          expect(result).toBe(path.join(appData, 'qaspec'));
        }
      }
    });
  });
  ...
  describe('getGlobalConfigPath', () => {
    it('should return path to config.json in config directory', () => {
      ...
      expect(result).toBe(path.join(tempDir, 'qaspec', 'config.json'));
    });
  });
  ...
  describe('getGlobalDataDir', () => {
    it('should use POSIX separators for Unix-like platform overrides', () => {
      ...
      ).toBe('/home/tabish/.local/share/qaspec');
    ...
    it('should use Windows separators for native Windows platform overrides', () => {
      ...
      ).toBe('C:\\Users\\Tabish\\AppData\\Local\\qaspec');
      ...
      ).toBe('D:\\Users\\Tabish\\AppData\\Local\\qaspec');
    });
  });
  ...
  describe('getGlobalConfig', () => {
    ...
    it('should not create directory when reading non-existent config', () => {
      process.env.XDG_CONFIG_HOME = tempDir;
      const configDir = path.join(tempDir, 'qaspec');
  ...
  describe('saveGlobalConfig', () => {
    ...
    it('should write config to file', () => {
      process.env.XDG_CONFIG_HOME = tempDir;
      const configPath = path.join(tempDir, 'qaspec', 'config.json');
