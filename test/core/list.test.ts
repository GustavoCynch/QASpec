...
describe('ListCommand', () => {
  ...
  describe('execute', () => {
    it('should handle missing qaspec/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No QASpec changes directory found. Run 'qaspec init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'qaspec', 'changes');
