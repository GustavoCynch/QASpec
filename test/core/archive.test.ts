...
describe('ArchiveCommand', () => {
  ...
  beforeEach(async () => {
    ...
    const openspecDir = path.join(tempDir, 'qaspec');
  ...
  describe('execute', () => {
    it('should archive a change successfully', async () => {
      ...
      const changeDir = path.join(tempDir, 'qaspec', 'changes', changeName);
      ...
      const archiveDir = path.join(tempDir, 'qaspec', 'changes', 'archive');
...
Then expected result happens`;
      ...
      const mainSpecPath = path.join(tempDir, 'qaspec', 'specs', 'test-capability', 'spec.md');
...
### Requirement: Thumbnail Field`;
      ...
      const mainSpecPath = path.join(tempDir, 'qaspec', 'specs', 'gift-card', 'spec.md');
      const updatedContent = await fs.readFile(mainSpecPath, 'utf-8');
...
Modified content.`;
      ...
      const mainSpecPath = path.join(tempDir, 'qaspec', 'specs', 'new-capability', 'spec.md');
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, 'qaspec', 'specs', 'another-capability', 'spec.md');
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
    ...
    it('should throw error if archive already exists', async () => {
      ...
      const archivePath = path.join(tempDir, 'qaspec', 'changes', 'archive', `${date}-${changeName}`);
...
Then expected result happens`;
    ...
    it('should support header trim-only normalization for matching', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'qaspec', 'specs', 'alpha');
...
Updated details.`;
    ...
    it('should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'qaspec', 'specs', 'beta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it('should abort with error when MODIFIED/REMOVED reference non-existent requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'qaspec', 'specs', 'gamma');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Another Missing`;
    ...
    it('should abort with a structural error when target spec hides requirements outside ## Requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'qaspec', 'specs', 'delta-target');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** qux changes`;
    ...
    it('should require MODIFIED to reference the NEW header when a rename exists (error format)', async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, 'qaspec', 'specs', 'delta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
new body`;
    ...
    it('should process multiple specs atomically (any failure aborts all)', async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, 'qaspec', 'specs', 'epsilon', 'spec.md');
...
e1`);
      ...
      const zetaMain = path.join(tempDir, 'qaspec', 'specs', 'zeta', 'spec.md');
...
### Requirement: Missing`);
    ...
    it('should display aggregated totals across multiple specs', async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, 'qaspec', 'specs', 'omega', 'spec.md');
      await fs.mkdir(path.dirname(omegaMain), { recursive: true });
      await fs.writeFile(omegaMain, `# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1`);

      const psiMain = path.join(tempDir, 'qaspec', 'specs', 'psi', 'spec.md');
  ...
  describe('error handling', () => {
    it('should throw error when qaspec directory does not exist', async () => {
      // Remove openspec directory
      await fs.rm(path.join(tempDir, 'qaspec'), { recursive: true });
  ...
  describe('interactive mode', () => {
    it('should use select prompt for change selection', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'qaspec', 'changes', change1), { recursive: true });
      await fs.mkdir(path.join(tempDir, 'qaspec', 'changes', change2), { recursive: true });
