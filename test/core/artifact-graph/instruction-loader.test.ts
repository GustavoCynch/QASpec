...
describe('instruction-loader', () => {
  ...
  describe('loadChangeContext', () => {
    ...
    it('should detect completed artifacts', () => {
      ...
      const changeDir = path.join(tempDir, 'qaspec', 'changes', 'my-change');
    ...
    it('should auto-detect schema from .qaspec.yaml metadata', () => {
      // Create change directory with metadata file
      const changeDir = path.join(tempDir, 'qaspec', 'changes', 'my-change');
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(path.join(changeDir, '.qaspec.yaml'), 'schema: spec-driven\ncreated: "2025-01-05"\n');
...
`
    ...
    describe('qaspec-pr-review subagent mode injection', () => {
      function setupQaReviewProject(
        ...
        const configDir = path.join(root, 'qaspec');
        fs.mkdirSync(configDir, { recursive: true });
        ...
        const changeDir = path.join(root, 'qaspec', 'changes', 'qa-change');
        fs.mkdirSync(changeDir, { recursive: true });
        fs.writeFileSync(
...
`
      ...
      it('should deduplicate validation warnings within session', () => {
        ...
        try {
          ...
          const configDir = path.join(freshTempDir, 'qaspec');
          fs.mkdirSync(configDir, { recursive: true });
          fs.writeFileSync(
            path.join(configDir, 'config.yaml'),
            `schema: spec-driven
