...
describe('openspec CLI e2e basics', () => {
  ...
  describe('init command non-interactive options', () => {
    it('initializes with --tools all option', async () => {
      ...
      const claudeSkillPath = path.join(emptyProjectDir, '.claude/skills/qaspec-explore/SKILL.md');
      const cursorSkillPath = path.join(emptyProjectDir, '.cursor/skills/qaspec-explore/SKILL.md');
