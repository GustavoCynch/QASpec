...
## Purpose
Define `qaspec feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide a `qaspec feedback` command that creates a GitHub Issue in the QASpec repository (per `GITHUB_REPO_SLUG` in branding) using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.

#### Scenario: Simple feedback submission

- **WHEN** user executes `qaspec feedback "Great tool!"`
- **THEN** the system executes `gh issue create` with title "Feedback: Great tool!"
- **AND** the issue is created in the QASpec repository
...
- **WHEN** user executes `qaspec feedback "Title here" --body "Detailed description..."`
- **THEN** the system creates a GitHub Issue with the specified title
- **AND** the issue body contains the detailed description
- **AND** the issue body includes metadata (QASpec version, platform, timestamp)
...
The system SHALL provide a `/feedback` skill that guides agents through collecting and submitting user feedback, and the generated skill body SHALL reference only the `qaspec` CLI.
...
- **AND** submits via `qaspec feedback` command on user confirmation

#### Scenario: Skill body has no openspec CLI references

- **WHEN** the feedback skill template is generated
- **THEN** its instructions and compatibility metadata reference `qaspec feedback` and "Requires qaspec CLI"
- **AND** no instruction tells the agent to run an `openspec` command
...
- **WHEN** user types `qaspec fee<TAB>`
- **THEN** the shell completes to `qaspec feedback`

#### Scenario: Flag completion

- **WHEN** user types `qaspec feedback "msg" --<TAB>`
