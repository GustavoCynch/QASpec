...
- **AND** display getting started section with QASpec core hints (`/qsx:analyze`, `/qsx:cases`, `/qsx:publish`) when the effective profile is `core` or when active workflows include any of `analyze`, `cases`, or `publish`
