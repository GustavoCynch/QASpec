 ...
 * `qaspec completion install` explicitly. This script only
 * prints a one-line tip after npm install.
 *
 * The tip is suppressed when:
 * - CI=true environment variable is set
 * - QASPEC_NO_COMPLETIONS=1 environment variable is set
...
function shouldSkipInstallation() {
  ...
  if (process.env.QASPEC_NO_COMPLETIONS === '1') {
    return { skip: true, reason: 'QASPEC_NO_COMPLETIONS=1 set' };
  }
