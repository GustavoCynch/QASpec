...
function main() {
  ...
  const binRelInPkg = typeof pkg.bin === 'string' ? pkg.bin : Object.values(pkg.bin)[0];
  ...
  try {
    ...
    work = mkdtempSync(path.join(tmpdir(), 'qaspec-pack-check-'));
    log(`Temp dir: ${work}`);
    ...
    const binRel = path.join('node_modules', ...pkg.name.split('/'), binRelInPkg);
